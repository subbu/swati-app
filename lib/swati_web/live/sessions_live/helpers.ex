defmodule SwatiWeb.SessionsLive.Helpers do
  ...
  # Channel icon component with brand SVGs
  attr :channel, :string, required: true
  attr :class, :string, default: "size-4"

  def channel_icon(%{channel: "whatsapp"} = assigns) do
    ~H"""
  ...
  def channel_icon(%{channel: "instagram"} = assigns) do
    ~H"""
    <svg class={@class} viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  ...
  def channel_icon(%{channel: "email"} = assigns) do
    ~H"""
    <svg class={@class} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg">
      <path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75"/>
    </svg>
    """
  end

  def channel_icon(%{channel: "voice"} = assigns) do
    ~H"""
    <svg class={@class} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg">
  ...
  def channel_icon(%{channel: "sms"} = assigns) do
    ~H"""
    <svg class={@class} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg">
  ...
  def channel_icon(%{channel: "webchat"} = assigns) do
    ~H"""
    <svg class={@class} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg">
  ...
  def channel_icon(%{channel: "public"} = assigns) do
    ~H"""
    <svg class={@class} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg">
      <path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-1.605.42-3.113 1.157-4.418"/>
    </svg>
  ...
  def channel_badge(nil) do
    %{key: "unknown", label: "—", color: "primary"}
  end
  ...
  def channel_badge(channel) do
    ...
    case normalized do
      "voice" -> %{key: "voice", label: "Voice", color: "info"}
      "sms" -> %{key: "sms", label: "SMS", color: "warning"}
      "whatsapp" -> %{key: "whatsapp", label: "WhatsApp", color: "success"}
      "instagram" -> %{key: "instagram", label: "Instagram", color: "danger"}
      "email" -> %{key: "email", label: "Email", color: "primary"}
      "webchat" -> %{key: "webchat", label: "Webchat", color: "info"}
      "slack" -> %{key: "slack", label: "Slack", color: "danger"}
      "telegram" -> %{key: "telegram", label: "Telegram", color: "info"}
      "public" -> %{key: "public", label: "Public", color: "success"}
      "" -> %{key: "unknown", label: "—", color: "primary"}
      _ -> %{key: normalized, label: titleize_channel(normalized), color: "info"}
    end
  end
