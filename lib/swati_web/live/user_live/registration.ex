defmodule SwatiWeb.UserLive.Registration do
  ...
  def mount(_params, _session, %{assigns: %{current_scope: %{user: user}}} = socket)
    ...
    {:ok, redirect(socket, to: SwatiWeb.UserAuth.signed_in_path(socket))}
  end
        ...
        case Accounts.deliver_login_instructions(
          ...
          {:error, _reason} ->
            {:noreply,
             ...
             |> put_flash(
               :error,
               "Account created, but we couldn't send the email. Please try logging in again."
             )
             |> push_navigate(to: ~p"/users/log-in")}
        end
