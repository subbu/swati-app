defmodule Swati.Accounts do
  ...
  alias Swati.Accounts.{Scope, User, UserNotifier, UserToken}
  ...
  def change_membership_invite(attrs \\ %{}) do
    Memberships.change_membership_invite(attrs)
  end
  ...
  def list_members(%Scope{} = current_scope) do
    Memberships.list_members(current_scope)
  end
  ...
  def invite_member(%Scope{} = current_scope, attrs, invite_url_fun)
    ...
    Memberships.invite_member(current_scope, attrs, invite_url_fun)
  end

  @doc """
  Returns whether the current scope is allowed to perform an action.
  """
  def authorized?(current_scope, action) do
    Memberships.authorized?(current_scope, action)
  end
  ...
  def change_user_registration(user, attrs \\ %{}, opts \\ []) do
    Registration.change_user_registration(user, attrs, opts)
  end
  ...
  def update_user_password(user, attrs) do
    ...
    |> Session.update_user_and_delete_all_tokens()
  end
  ...
  def generate_user_session_token(user) do
    Session.generate_user_session_token(user)
  end
  ...
  def get_user_by_session_token(token) do
    Session.get_user_by_session_token(token)
  end
  ...
  def get_user_by_magic_link_token(token) do
    MagicLink.get_user_by_magic_link_token(token)
  end
  ...
  def login_user_by_magic_link(token) do
    MagicLink.login_user_by_magic_link(token)
  end
  ...
  def deliver_login_instructions(%User{} = user, magic_link_url_fun)
    ...
    MagicLink.deliver_login_instructions(user, magic_link_url_fun)
  end
