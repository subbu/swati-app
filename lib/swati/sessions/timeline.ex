defmodule Swati.Sessions.Timeline do
  ...
  def upsert(session_id, payload) when is_binary(session_id) and is_map(payload) do
    now = DateTime.utc_now() |> DateTime.truncate(:microsecond)

    meta = build_meta(session_id, payload, now)
    utterances = build_utterances(session_id, payload, now)
    segments = build_segments(session_id, payload, now)
    tool_calls = build_tool_calls(session_id, payload, now)
    markers = build_markers(session_id, payload, now)

    Repo.transaction(fn ->
      Repo.insert_all(SessionTimelineMeta, [meta],
        on_conflict:
          {:replace, [:origin_ts, :origin_type, :duration_ms, :version, :built_at, :updated_at]},
        conflict_target: [:session_id]
      )

      Repo.delete_all(from(u in SessionUtterance, where: u.session_id == ^session_id))
      Repo.delete_all(from(s in SessionSpeakerSegment, where: s.session_id == ^session_id))
      Repo.delete_all(from(t in SessionToolCall, where: t.session_id == ^session_id))
      Repo.delete_all(from(m in SessionMarker, where: m.session_id == ^session_id))

      insert_all(SessionUtterance, utterances)
      insert_all(SessionSpeakerSegment, segments)
      insert_all(SessionToolCall, tool_calls)
      insert_all(SessionMarker, markers)
    end)
  ...
  defp build_meta(session_id, payload, now) do
  ...
  defp build_utterances(session_id, payload, now) do
  ...
  defp build_segments(session_id, payload, now) do
  ...
  defp build_tool_calls(session_id, payload, now) do
  ...
  defp build_markers(session_id, payload, now) do
